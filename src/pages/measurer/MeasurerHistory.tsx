...
import { mockRequests, statusLabels, statusColors, type ServiceRequest } from "@/data/mockDashboard";
import { Search, Calendar, MapPin, ChevronDown, ChevronUp } from "lucide-react";

const allCompleted = mockRequests.filter((r) => r.assignedTo === "Сидоров К.В." && (r.status === "closed" || r.status === "measurement_done"));
                    ...
                    <div className="mt-3 pt-3 border-t border-border grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      ...
                      {r.files && r.files.length > 0 && (
                        <div className="sm:col-span-2">
                          <p className="text-xs text-muted-foreground mb-1">Файлы:</p>
                          <div className="flex flex-wrap gap-1.5">
                            {r.files.map((f, i) => (
                              <span key={i} className="px-2 py-1 bg-accent rounded text-xs">{f}</span>
                            ))}
                          </div>
                        </div>
                      )}
