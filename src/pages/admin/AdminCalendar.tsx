...
const AdminCalendar = () => {
  ...
  // Group installation requests by agreed_date
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-4">
                      ...
                      <div className="mt-0.5 md:mt-1">
                        ...
                        <div className="hidden md:flex flex-col gap-0.5">
                          {dayRequests.slice(0, 2).map((r) => (
                            <div
                              key={r.id}
                              className="text-[10px] leading-tight truncate px-1 py-0.5 rounded bg-primary/10 text-primary font-medium"
                            >
        ...
        {loading && (
          <p className="text-center text-sm text-muted-foreground">Загрузка заявок...</p>
        )}
      </div>
        ...
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          {selectedDateRequests.length === 0 ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              Нет запланированных монтажей на этот день
            </p>
          ) : (
            <div className="space-y-3">
              {selectedDateRequests.map((r) => (
                <div
                  key={r.id}
                  className="border border-border/50 rounded-xl p-3 space-y-2 hover:bg-accent/30 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono text-muted-foreground">{r.number}</span>
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                        statusColors[r.status as RequestStatus] || "bg-gray-100 text-gray-600"
                      }`}
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <a href={`tel:${r.client_phone}`} className="hover:text-primary">
                        {r.client_phone}
                      </a>
                    </div>
